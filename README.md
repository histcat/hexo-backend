...
hexo-backend/
├── api/
│   └── index.ts             # Vercel Serverless Function 入口，转发所有 /api/* 请求（vercel.json rewrite）
...
1. 文章列表显示配置目录下的所有 `.md` / `.mdx` 文件，支持按分类 / 标签 / 状态（全部 / 草稿 / 已发布）筛选和搜索，草稿带黄色「草稿」徽标；
...
5. 支持「存草稿」与「发布」：存草稿即保存为 `draft: true` 的文章（博客构建不会发布它），发布则清除草稿标志；
6. 草稿存储在博客仓库中（云端接力）：换设备登录后台，在「草稿」筛选里就能找到并继续编辑；
7. 支持重命名 / 移动 / 删除文章。

### 4. 编辑配置文件

1. 在文章列表页切换到「配置文件」Tab；
2. 列表只显示 `src/config.ts` 与 `src/data/friends.ts` 两个文件；
3. 在线编辑并保存（TypeScript 按纯文本编辑，原样提交）。
...
这是前端调用 `/api/*` 时收到了 Vercel 的 500 错误页（不是 JSON）。常见原因与对应修复：

1. **相对导入带 `.ts` 后缀（根本原因）**：Vercel 会把 `api/`、`server/` 下的 TypeScript 逐文件编译为 `.js`，但不会改写 import 里的 `.ts` 后缀，运行时找不到模块（`ERR_MODULE_NOT_FOUND`），所有 API 请求都会返回 `FUNCTION_INVOCATION_FAILED`。本项目已把所有相对导入改为 `.js` 后缀。
2. **函数入口**：Vercel 普通函数通过 default export 调用，入口文件应使用 `export default handle(app)`。
3. **运行时兼容**：`package.json` 中 `engines.node >= 20`，且已包含 Web Crypto 兼容修复（Node 18 缺少全局 `crypto`）。

如仍复现，可在 Vercel 项目的 **Functions** 日志中查看具体报错。
...
确认 `vercel.json` 存在且 `api/index.ts` 未被删除；`/api/*` 请求由 rewrite 转发到该 Serverless Function，其余路径回退到 `index.html`（SPA 路由）。
